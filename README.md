...
- Assinatura digital com hash SHA-256 do conteúdo. Validação por PIN está especificada no backend, mas ainda não exposta na interface atual (ver `turno-qa-suite`)
...
**Dashboard do gestor**
- Resumos das passagens do dia como primeiro elemento da tela
...
- Fluxo de convite completo implementado (cadastro, cargo, canal de contato)
...
Row Level Security no Supabase implementado para isolamento entre organizações. O processo de QA (ver `turno-qa-suite`) identificou uma falha na resolução de identidade do usuário autenticado que compromete esse isolamento em cenários específicos; investigação e correção em andamento.
